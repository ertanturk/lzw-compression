...
def _encode_decode_rgb_via_container(
	rgb: np.ndarray, use_differences: bool
) -> np.ndarray:
	...
	if use_differences:
		red_decoded = codes_to_image_grayscale_differences(
			red_loaded_codes, (h, w)
		)
		green_decoded = codes_to_image_grayscale_differences(
			green_loaded_codes, (h, w)
		)
		blue_decoded = codes_to_image_grayscale_differences(
			blue_loaded_codes, (h, w)
		)
	else:
	...
	return np.stack((red_decoded, green_decoded, blue_decoded), axis=2).astype(
		np.uint8
	)
