...
def image_file_encoder_grayscale(file_path: str) -> list[int]:
	...
	try:
		...
		print(
			f"An error occurred during encoding image file '{file_path}': {e}",
		)
		sys.exit(1)
...
def encode_grayscale_array_lzw_with_differences(image_array: np.ndarray) -> list[int]:
	...
	try:
		...
		diff_array_offset: np.ndarray = (
			(diff_array.astype(int) + 128) % 256
		).astype(int)
		...
		print(
			f"An error occurred during encoding grayscale image array with differences: {e}",
		)
		sys.exit(1)
...
def image_file_encoder_grayscale_differences(file_path: str) -> list[int]:
	...
	try:
		...
		print(
			f"An error occurred during encoding image file with differences '{file_path}': {e}",
		)
		sys.exit(1)
