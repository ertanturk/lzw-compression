...
def write_bitstream_to_text_file(bitstream: bytes, output_file_path: str) -> None:
	...
	try:
		...
		print(
			f"An error occurred while writing to the file '{output_file_path}': {e}",
		)
		sys.exit(1)
...
def open_color_bitstreams_with_dimensions(
	file_path: str,
...
def save_image_file(
	...
	try:
		...
		print(
			f"An error occurred while saving the image '{output_file_path}': {e}",
		)
		sys.exit(1)
