...
def text_file_decoder(file_path: str) -> list[int]:
    ...
    try:
        ...
        print(f"An error occurred during decoding bitstream file '{file_path}': {e}")
        sys.exit(1)
...
def codes_to_image_grayscale(codes: list[int], image_shape: tuple[int, int]) -> np.ndarray:
...
def image_file_decoder_grayscale(file_path: str) -> np.ndarray:
    ...
    try:
        ...
        bitstream, height, width = open_bitstream_file_with_dimensions(file_path)
        ...
        print(f"An error occurred during decoding image file '{file_path}': {e}")
        sys.exit(1)
...
def codes_to_image_grayscale_differences(
    ...
    try:
        ...
        for row in range(1, image_shape[0]):
            for col in range(1, image_shape[1]):
                image[row, col] = (image[row, col - 1] + delta_pixels[row, col]) % 256
...
def image_file_decoder_grayscale_differences(file_path: str) -> np.ndarray:
    ...
    try:
        ...
        print(f"An error occurred during decoding delta image file '{file_path}': {e}")
        sys.exit(1)
