...
from lzw_compression.core.bitstream import (
...
class LZWCompressionApp:
	...
	def _build_ui(self) -> None:  # noqa: PLR0915
		...
		# -- method --
		frm_method = ttk.LabelFrame(
			frm_opts,
			text="2 - Encoding Method",
			padding=8,
		)
	...
	def _show_image_preview(self, arr: np.ndarray) -> None:
		...
		self._lbl_image.config(image=photo, text="")
		self._lbl_image.image = photo  # type: ignore[attr-defined]
		self._notebook.select(self._frm_img_tab)  # type: ignore[no-untyped-call]
	...
	def _show_text_preview(self, text: str) -> None:
		...
		self._notebook.select(self._frm_txt_tab)  # type: ignore[no-untyped-call]
	...
	def _on_open_file(self) -> None:
		...
		try:
			...
			if self._is_text:
				...
				has_color = (
					len(self._image_array.shape)
					== _MIN_RGB_CHANNELS
					and self._image_array.shape[2]
					>= _MIN_RGB_CHANNELS
				)
	...
	def _on_compress(self) -> None:
		...
		if self._src_path is None:
			messagebox.showwarning(
				"Nothing loaded", "Please open a file first."
			)
			return
		...
		try:
			...
			if self._is_image:
				...
				if is_color:
					self._compress_color(save_path, method)
				else:
					self._on_channel_changed()
					if self._active_array is None:
						...
						return
					self._compress_grayscale(save_path, method)

		except Exception as exc:
			messagebox.showerror("Compression Error", str(exc))
			self._set_status("Compression failed")

	def _compress_text(self, save_path: str) -> None:
		...
		self._set_status(f"Saved compressed text -> {Path(save_path).name}")
		messagebox.showinfo("Done", "Text compressed successfully.")

	def _compress_color(self, save_path: str, method: str) -> None:
		assert self._src_path is not None
		...
		assert self._image_array is not None
		h, w = self._image_array.shape[:2]
		write_color_bitstreams_with_dimensions(
			bitstream_r, bitstream_g, bitstream_b, save_path, h, w
		)
		...
		messagebox.showinfo(
			"Done", "Color image compressed successfully (R/G/B channels)."
		)

	def _compress_grayscale(self, save_path: str, method: str) -> None:
		assert self._active_array is not None
		if method == "differences":
			codes = encode_grayscale_array_lzw_with_differences(
				self._active_array
			)
		else:
	...
	def _on_decompress(self) -> None:
		...
		try:
			if self._try_decompress_color(path, method):
				return
			if self._try_decompress_image(path, method):
				return
			if self._try_decompress_text(path, method):
				return
		except Exception as exc:
			messagebox.showerror("Decompression Error", str(exc))
			self._set_status("Decompression failed")

	def _try_decompress_color(self, path: str, method: str) -> bool:
		try:
			red_bs, green_bs, blue_bs, height, width = (
				open_color_bitstreams_with_dimensions(path)
			)
		except Exception:
			return False
		...
		if method == "differences":
			red = codes_to_image_grayscale_differences(
				r_codes, (height, width)
			)
			green = codes_to_image_grayscale_differences(
				g_codes, (height, width)
			)
			blue = codes_to_image_grayscale_differences(
				b_codes, (height, width)
			)
		else:
	...
	def _try_decompress_image(self, path: str, method: str) -> bool:
		try:
			bitstream_raw, height, width = (
				open_bitstream_file_with_dimensions(path)
			)
		except Exception:
			return False

		if not (
			0 < height < _MAX_IMAGE_DIMENSION
			and 0 < width < _MAX_IMAGE_DIMENSION
		):
			return False
		...
		if method == "differences":
			...
			decoded_img = codes_to_image_grayscale(codes, (height, width))
		...
		self._display_metrics_image(tmp_decoded, path)
		self._set_status(f"Decompressed image ({height}x{width})")
		return True

	def _try_decompress_text(self, path: str, method: str) -> bool:
		try:
		...
		self._display_metrics_text_from_sizes(
			original_size=len(text.encode("utf-8")), compressed_path=path
		)
	...
	def _display_metrics_text_from_sizes(
		...
		ss = (
			(original_size - comp_size) / original_size
			if original_size
			else 0.0
		)
	...
	def _display_metrics_image(
		...
		metric_array = (
			...
			else self._active_array
		)

		if metric_array is not None:
			entropy_original = calculate_entropy(metric_array.flatten())
			entropy: float | str = entropy_original
			if self._method_var.get() == "differences":
				if len(metric_array.shape) == _TWO_D:
					diff = compute_left_top_differences_2d(
						metric_array,
					)
					diff_offset = (
						(diff.astype(int) + 128) % 256
					).astype(np.uint8)
					entropy_diff = calculate_entropy(
						diff_offset.flatten(),
					)
				else:
			...
			entropy = 0.0
		symbol_count = (
			int(metric_array.size)
			if metric_array is not None
			else len(self._codes)
		)
		acl = calculate_average_code_length(
			self._bitstream,
			self._codes,
	...
	def _set_metric_values(  # noqa: PLR0913
		...
		*,
		...
		if isinstance(entropy, str):
			...
			self._metric_labels["entropy"].config(
				text=f"{entropy:.4f} bits",
			)
	...
	def _on_clear(self, silent: bool = False) -> None:
		...
		self._lbl_image.config(image="", text="No image")
